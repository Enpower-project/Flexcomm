...
def classify_residuals_ternary_rescaled(
    ...
    # robust daily scale (avoid spikes)
    scale = np.nanpercentile(resids, scale_pct, axis=1, keepdims=True)
    scale = np.where(scale <= 0, np.nan, scale)

    active_thr = np.maximum(min_active_abs, active_ratio * scale)
    high_thr   = np.maximum(min_high_abs,   high_ratio  * scale)

    active = resids >= active_thr
    high   = resids >= high_thr

    out = np.zeros_like(resids, dtype=dtype)
    out[active] = 1
    out[high] = 2
    return out
...
def run_hvac_disaggregation(df, load_col="total_consumption", temp_col="Tout",
                            ...
                            min_high_abs=1000, q=0.20, smooth=3, min_days_per_bin=3, min_act_abs=0.5, slots_per_day=48):
    ...
    hvac_ternary = classify_residuals_ternary_rescaled(
        ...
        min_active_abs=min_act_abs,  
        min_high_abs=min_high_abs
    )
    hvac_mode = pd.Series(
        0,
        index=df.index,
    ...
    # df_with_hvac.to_csv('df_temp.csv')
