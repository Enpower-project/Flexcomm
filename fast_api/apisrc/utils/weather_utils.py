...
from utils.timezone_utils import get_site_timezone
...
def fetch_environmental_data(
    *,
    ...
    site_tz=None,  # Optional: ZoneInfo object for timezone conversion
):
    """
    Fetch hourly environmental data from Open-Meteo.

    Args:
        site_tz: ZoneInfo object for the site's timezone (used to convert local times to UTC)
    ...
    """
    # Get site timezone if not provided
    if site_tz is None:
        site_tz = get_site_timezone(latitude, longitude)
    ...
    for t, temp, rh, sol in zip(times, temps, rhs, solar):
        ...
        # Convert to UTC for database storage (naive UTC)
        if ts.tzinfo is not None:
            ...
            ts = ts.replace(tzinfo=site_tz).astimezone(timezone.utc).replace(tzinfo=None)
...
def reconcile_environmental_data(
    *,
    ...
    # Get site timezone for proper timestamp conversion
    site_tz = get_site_timezone(latitude, longitude)
    ...
    forecast_cutoff = now_utc - timedelta(days=5)
